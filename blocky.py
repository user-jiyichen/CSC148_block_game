...
def _block_to_squares(board: Block) -> List[Tuple[Tuple[int, int, int],
    ...
    """
    if len(board.children) == 0:  # the block can not be subdivided
        colour = board.colour
        position = board.position
        size = board.size
        info = (colour, position, size)
        return [info]
    else:  # len(board.children) == 4
        lst = []
...
class MainState(GameState):
    ...
    def process_event(self, event: pygame.event.Event) -> None:
        """Process the event of the player from the operating system.
        """
        self._current_player().process_event(event)

    def update(self) -> GameState:
        """Update this GameState based on past moves of the player.
        Return the next GameState that should be updated.
        Stay in the current state if no move was made.
        """
...
class AnimateMoveState(GameState):
    ...
    def update(self) -> GameState:
        """Update this AnimateMoveStates and return it.
        """
    ...
    def render(self, renderer: Renderer) -> None:
        ...
        The outline of the selected block, the image representing the move
        should be drawn, then update the status message.
        """
...
class GameOverState(GameState):
    ...
    def process_event(self, event: pygame.event.Event) -> None:
        """No events should be processed. Return None."""
    ...
    def render(self, renderer: Renderer) -> None:
        """Render the current state of the game onto the screen.
        Show the final score and the winner of the players.
        """
        x = 10
        y = 10
